...
const signupPagePost = async (req, res)=>{
	...
	const birthDate = req.body.birthDateInput.trim();
	...
	if(!validator.isEmail(email)) {
		let notice = "Emaili ei ole korrektne!";
	    console.log(notice);
	    return res.render("signup", {notice: notice});
	}
	
	//kas sünnikuupäev on korrektne/reaalne
	...
	try {
		...
		if(users.length > 0){
			let notice = "See email on juba kasutusel!";
			console.log(notice);
			return res.render("signup", {notice: notice});
		}
		//krüpteerime parooli
		...
		console.log(err);
		res.render("signup", {notice: "Tehniline viga! Proovi hiljem uuesti."});
	}
	finally {
		/* if(conn){
			await conn.end();
